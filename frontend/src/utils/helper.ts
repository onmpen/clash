...
// GitHub download mirror prefixes and fallback helper
export const GH_PROXY_PREFIXES: readonly string[] = [
  'https://edgeone.gh-proxy.com/',
  'https://cdn.gh-proxy.com/',
]

export const buildGhProxyUrls = (originUrl: string): string[] => {
  const trimmed = originUrl.trim()
  const mirrors = GH_PROXY_PREFIXES.map((p) => `${p}${trimmed}`)
  // ensure origin is tried last as ultimate fallback
  return [...mirrors, trimmed]
}

/**
 * Try to download a URL via multiple GitHub mirrors with automatic fallback.
 * Keeps the same signature behavior as Download for headers/progress/options that we actually use.
 */
export const downloadWithGhMirrors = async (
  originUrl: string,
  savePath: string,
  onProgress?: (progress: number, total: number) => void,
  options?: Recordable,
) => {
  const urls = buildGhProxyUrls(originUrl)
  let lastErr: any
  for (const url of urls) {
    try {
      await Download(url, savePath, undefined, onProgress, options as any)
      return url
    } catch (err) {
      lastErr = err
      await ignoredError(RemoveFile, savePath)
      // try next mirror
    }
  }
